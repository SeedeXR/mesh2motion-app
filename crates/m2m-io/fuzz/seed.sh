...
repo="$here/../../.."
for target in fbx_binary fbx_text fbx_pipeline; do
...
echo "corpus: binary=$(ls "$here/corpus/fbx_binary" | wc -l | tr -d ' ') text=$(ls "$here/corpus/fbx_text" | wc -l | tr -d ' ') pipeline=$(ls "$here/corpus/fbx_pipeline" | wc -l | tr -d ' ')"
