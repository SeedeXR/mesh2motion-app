...
- [ ] ITEM 2b (FBX material/texture reading): the on-theme remaining work — an FBX source keeps UVs but takes the default material. Needs an fbx material reader: Material(DiffuseColor) via OP-connected Texture via OO-connected Video(Content=embedded bytes / RelativeFilename external). dom.rs exposes children_of/parents_of (OO+OP) + objects_of_kind + Object.property. Round-trip verifiable against our own textured_rig.fbx (embedded Video). Then convert.rs -> Document.materials + primitive.material; export re-emits. LARGE (dom docs note materials are ~46% of the format, deliberately unparsed).
- [ ] ITEM 3 (multi-material FBX): niche. glTF->glb already keeps all materials (graft). glTF->fbx merges primitives into one mesh w/ first material. Needs per-polygon LayerElementMaterial + multiple Material/Texture/Video + a multi-material fixture.
